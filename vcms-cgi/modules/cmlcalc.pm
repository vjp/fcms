...
sub execute 	{
 		...
 		unless($METHODID->{script}) {
 			...
 			$res=encu($low?"Метод нижних объектов $method не найден":"Метод $method не найден");
 			if (
 		...
 		if ($@) {
 			if (
 				...
 				&cmlmain::alert(encu("Ошибка выполнения метода $method : $@"));
 			}	 
 		} else {
 			$res=$ev
 		}
...
sub baselparser (;$)
{
	...
	for my $cgiprm (keys %$CGIPARAM) {
		...
		if ($cgiprm=~/^_o(.+?)_p(.+)/) {
			...
			push (@{$changed->{$id}},$CGIPARAM->{fileprm});
		}
	}
	...
	unless ($opts->{silent} || $CGIPARAM->{silent}) {
		...
		$alerttext=encu('Значения изменены') unless $alerttext;
		alert($alerttext);
	}	
